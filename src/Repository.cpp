...
#include<cmath>
#include<set>
Database<std::array<char,20>,Book,50> isbn_db;// 存放ISBN和Book结构体的映射
Database<std::array<char,60>,std::array<char,20>,50> name_isbn_map;// BookName到ISBN的映射
Database<std::array<char,60>,std::array<char,20>,50> author_isbn_map;// Author到ISBN的映射
Database<std::array<char,60>,std::array<char,20>,50> keyword_isbn_map;// Keyword到ISBN的映射
...
void Repository::Parser(std::string line,std::string& type,std::vector<std::string>& index) {
    if (line[0] == '-') {
        ...
        if (type == "ISBN") {
            ...
            if (tmp.size() <= 20 && !tmp.empty()) index.push_back(tmp);
        }
        else if (type == "name") {
